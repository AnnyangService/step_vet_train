...
def train_disease(disease):
    """특정 질병에 대한 GAN 학습을 수행하는 함수"""
    print(f"\n{disease} 학습 시작...")
    
    # GAN 학습 매개변수 설정
    outdir = f"/home/minelab/desktop/Jack/step_vet_train/models/projected_gan/{disease}"
    cfg = "fastgan"
    data = f"/home/minelab/desktop/Jack/step_vet_train/datasets/training_gan/{disease}"
    gpus = 2  
    batch = 64 
    mirror = 1
    snap = 50
    ...
    # 특정 체크포인트 파일 직접 지정
    checkpoint_path = f"/home/minelab/desktop/Jack/step_vet_train/datasets/training_gan/{disease}/00000-fastgan-{disease}-gpus2-batch64/best_model.pkl"
    ...
    if success:
        print(f"{disease} GAN 학습이 성공적으로 완료되었습니다.")
    else:
        print(f"{disease} GAN 학습 중 오류가 발생했습니다.")
...
def main():
    ...
    for disease in diseases:
        success = train_disease(disease)
        if not success:
            print(f"{disease} 학습 중 오류가 발생하여 프로그램을 종료합니다.")
            return False
    
    print("\n모든 질병에 대한 GAN 학습이 완료되었습니다!")
    return True
